...
public class GetEventsByFairUseCaseTest {
    ...
    @Mock
    ApiUseCase.Listener<FairEvent[]> listener;
    ...
    public void it_successfully_fetches_fair_events_by_fair() {
        ...
        String fairId                   = "1";
        FairEvent[] fairEvents          = new FairEvent[] {};
        int statusCode                  = 200;
        Map<String, String> parameters  = new HashMap<>();

        Call<FairEvent[]> call                                  = networkMockHelper.getCallMock();
        Response<FairEvent[]> response                          = networkMockHelper.getResponseMock();
        ArgumentCaptor<Callback<FairEvent[]>> argumentCaptor    = networkMockHelper.getResponseCallbackCaptor();

        when(apiClientMock.getFairEventsByFair(fairId, parameters)).thenReturn(call);

        when(response.body()).thenReturn(fairEvents);
        ...
        verify(listener).onResponse(statusCode, fairEvents);
    }
