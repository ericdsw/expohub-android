...
public class UnAttendEventUseCaseTest extends BaseApiUseCaseTest {

    @Mock
    ApiUseCase.Listener<ResponseBody> listener;

    private UnAttendFairEventUseCase useCase;
    private ApiClientRequestCallback<ResponseBody, UnAttendFairEventUseCase> genericCallback;
    ...
    public void setUp() {
        ...
        useCase = new UnAttendFairEventUseCase(apiClientMock, moshiMock);
        genericCallback = new ApiClientRequestCallback<ResponseBody, UnAttendFairEventUseCase>() {
            @Override
            public void setupExecutedApiRequest(ApiClient apiClient, Map<String, String> parameters, Call<ResponseBody> call) {
                when(apiClient.unAttendFairEvent(fairEventId)).thenReturn(call);
            }

            @Override
            public void triggerRequest(UnAttendFairEventUseCase apiUseCase) {
