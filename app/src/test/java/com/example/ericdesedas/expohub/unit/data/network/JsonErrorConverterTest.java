...
public class JsonErrorConverterTest {
    ...
    public void it_correctly_parses_json_api_error_with_one_error() throws IOException {
        ...
        assertThat("Checking that ApiErrorWrapper's first error title parsed correctly", apiErrorWrapper.errorList.get(0).getTitle(), is("foo"));
        assertThat("Checking that ApiErrorWrapper's first error message parsed correctly", apiErrorWrapper.errorList.get(0).getDetail(), is("bar"));
        assertThat("Checking that ApiErrorWrapper's first error status parsed correctly", apiErrorWrapper.errorList.get(0).getStatus(), is("400"));
    }
